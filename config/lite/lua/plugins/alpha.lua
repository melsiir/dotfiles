...
function Plugin.opts()
  ...
  section.footer = {
    type = "text",
    val = string.format("v%s.%s.%s", version.major, version.minor, version.patch),
    opts = {
      position = "center",
      hl = "comment",
    },
  ...
  autocmd("User", {
    ...
    callback = function()
      for _, item in pairs(opts.action) do
        vim.keymap.set("n", item.keys, item.fn, { silent = true, buffer = true, nowait = true })
      end
