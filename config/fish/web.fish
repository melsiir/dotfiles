...
function chrome -d "open chrome"
    if test (count $argv) -eq 0
        am start -n com.android.chrome/com.google.android.apps.chrome.Main --activity-clear-task >/dev/null
    else
        am start -n com.android.chrome/org.chromium.chrome.browser.ChromeTabbedActivity -d "$argv" --activity-clear-task >/dev/null
    end
end
...
function jsonValue -d "extract value from json object by providing the key as argument"
    if test -z $argv[2]
        echo "help:"
        echo "  jsonValue [file or variable] key"
        return
    end
    if test -e $argv[1]
        set jsonText (cat $argv[1])
        set found (echo $jsonText | grep -oP "$argv[2](.*?)," | string replace "$argv[2]\":" '' \
            | string replace -a "\"" "" | string replace "," "")
        if test -z $jsonText
            echo "the file $argv[1] is empty!"
            return
        end
        echo $found
        return
    end
    set found (echo $argv[1] | grep -oP "$argv[2](.*?)," | string replace "$argv[2]\":" '' \
            | string replace -a "\"" "" | string replace "," "")
...
function send -d "send files to file.io"

    if test -z $argv
        echo "please provide the file to upload"
        return
    end

    set respones (curl --progress-bar -F "file=@$argv[1]" -s -w "\n" https://file.io/?maxDownloads=1000)

    if test -z $respones
        echo -e "$RED warning!$RC"
        echo -e "it seem like you cannot access file.io server"
        echo "please check you internet connection"
        return
    end

    set link (jsonValue $respones link)
    set jsonStatus (jsonValue $respones status)
    set expireDate (jsonValue $respones expires | grep -Po "(\d){4}-(\d){2}-(\d){2}")
    if test $jsonStatus -eq 200
        echo -e " file $argv[1] has been uploaded$GREEN$bold successfully$RC to file.io"
        echo " here is your link"
        echo -e " $GREEN$link$RC"
        echo -e " Expires at $expireDate"
    end

    ## jq method
    # curl --progress-bar -F "file=@$argv[1]" -s -w "\n" https://file.io | jq -r '.link'
    ## manual method
    # curl --progress-bar -F "file=@$argv[1]" -s -w "\n" https://file.io \
    #     | grep -oP "link(.*?)," | string replace 'link":"' '' \
    #     | string replace "\"," ""
end
