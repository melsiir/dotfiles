...
-- buffers
-- also use H to switch to next buffer and L to previous buffer natively
...
map("n", "<leader>ssr", function()
  local melsir = require "config.util.melsir"
  melsir.FormatForReddit()
end, { desc = "format for reddit post" })
...
map("n", "<leader>qq", "<cmd>q! <CR>", { desc = "quit without confirmation!" })
...
map("n", "<leader>yp", ':let @* = expand("%:p") | echo "yanked: " . @*<CR>',
...
map("n", "<leader>sf", ":%s/\\<<C-r><C-w>\\>/<C-r><C-w>/gI<Left><Left><Left>", { desc = "replace word" })
...
map("t", "<c-_>", "<cmd>close<cr>", { desc = "hide terminal" }) -- this actualy / not _  so C-/ in fact
------------------------------------------------
